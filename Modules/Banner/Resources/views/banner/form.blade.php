                ...
                <div class="card">
                    <div class="card-body">
                        <div class="panel panel-primary">
                            <input type="hidden" name="category_id" id="category_id"  value="{{ !empty($parent['id']) ? $parent['id'] : 1 }}">
                                        ...
                                        @foreach ($list['data_parent'] as $parent)
                                            @if (!empty($parent['id']))
                                                <option value="{{ $parent['id'] }}"
                                                    @if (!empty($data->category_id) == $parent['id'] && $data->category_id == $parent['id'])
                                                        selected 
                                                    @endif>
                                                        {{ $parent['name'] }}
                                                </option>
                            ...
                            {{-- <div class="form-check form-check-inline py-5 ">
                                <input class="form-check-input" type="radio" name="inlineRadioOptions" id="inlineRadio1" value="1"{{ !empty($data->type) ? $data->type == 1 ? "checked" : "" : "checked"  }}  onchange="ChkImageOrVdo(1);">
                                <label class="form-check-label" for="inlineRadio1">อัปโหลดรูปภาพ</label>
                            </div>
                            <div class="form-check form-check-inline py-5 inlineurlVideo {{ !empty($data->category_id) ? $data->category_id != 4 ? "" : "d-none" : "" }}">
                                <input class="form-check-input" type="radio" name="inlineRadioOptions" id="inlineRadio2" value="2" {{ !empty($data->type) ? $data->type == 2 ? "checked" : "" : "" }} onchange="ChkImageOrVdo(2);">
                                <label class="form-check-label" for="inlineRadio2">URL YouTube Video</label>
                            </div> --}}
                       
                            <div class="form-group image_banner {{ !empty($data->type) ? $data->type == 1 ? "" : "d-none" : "" }}" >
                                <?= image_upload($id = '1', $name = 'image', $label = __('websetting_admin.upload_image_header'), $image = !empty($data->image) ? $data->image : '', $size_recommend = '1920 × 855px',false,true,'ลบรูปภาพ','DeleteImage') ?>
                            </div>
