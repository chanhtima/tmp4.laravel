     ...
     */
         ...
         if ($request->ajax()) {
             ...
             $tables = Datatables::of($products)
                 ...
                 ->editColumn('images', function ($product) {
                     if (!empty($product['images']) && CheckFileInServer($product['images'])) {
                             return '<img src="' . $product['images'] . '" style="max-height: 60px" />';
                     }
                 })
     ...
     public function save(Request $request)
     {
       ...
       if(!empty($request->get('index_status'))){
         ...
         $delelte_temp = [];
        // upload image
        $image = set_image_upload($request, 'images', $path = "public/product", "image-");
        if ($image) {
