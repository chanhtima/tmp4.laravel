...
class SlugController extends Controller
{
    ...
    // กรณีไม่มี slug ให้นำ ชื่อมาใช้แทน ใช้ใน case ที่ไม่จำเป็นต้องใช้ slug
    public function add_slug_by_name(array $meta,$name_th = "",$name_en = ""){
        if(!empty($meta)){
            $text['th'] = ['name' => $name_th];
            $text['en'] = ['name' => $name_en];
            foreach($meta as $lang => $data){
            $metadata[$lang] = [
                "module" => $data['module'],
                "method" => $data['method'],
                "level" => $data['level'],
                "slug" => !empty($data['slug']) ? $data['slug'] : $text[$lang]['name'],
    ...
    // delete slug กรณีลบข้อมูล slug ลบออกด้วย
    public function delete_slug($module,$method,$data_id){
        $slugs = Slugs::where('module', $module)->where('method', $method)->where('data_id', $data_id)->get();
            if(!empty($slugs)){
